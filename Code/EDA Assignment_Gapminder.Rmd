...
## Question  Examine "typical" life expectancy for different years.
...
```{r}
## Check for missing values and removing the , from the population  the missing values with 0
is.na(ds$population)
...
p3 = ggplot(data = ds1951, aes(x = region, y = income)) +
  geom_boxplot(aes(fill = region)) +
  ggtitle("income across regions Year 1950") +
  xlab('regions') + ylab("income fixed PPP$") +
...
p4 = ggplot(data = ds2015, aes(x = region, y = income)) +
  ...
  xlab('Region') + ylab("Income fixed PPP$") +
  scale_y_log10(breaks = ybreaks,
                labels = comma(ybreaks)) +
  scale_fill_manual(values= Palette)
...
Analysis :Now we go about Finding the Correlation betweeen Life expectancy and income for 1951
...
Analysis :Now we go about Finding the Correlation betweeen Life expectancy and income for 2015
...
```{r}
##Find the max and min income for all continents in 2008
...
```{r}
##Lets select the base of low lifeExp as 60
...
##  QUESTION 6 Find countries with sudden, substantial departures from the temporal trend in income
```{r}
