...
Because anything that is `^ 0` = `1`.
Because NA | TRUE is saying whether one of the two is `TRUE`((boolean OR) and the second one is.
Because at least one of the two expressions can be tested: FALSE & NA. In NA & NA nothing can be tested and the results is `NA & NA`.
...
It will just return only one time
...
```
 ...
 By including inside `one_of()` allows us to use character names.
